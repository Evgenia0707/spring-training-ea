...
//Write a derived query to read a movie with a name
    ...
    Optional<Movie> findByName(String name); //List<Movie>
...
//Write a JPQL query to list all movies between a range of prices
    ...
    List<Movie> fetchAllMoviesBetweenPriceRange(@Param("price1") BigDecimal price1, @Param("price2") BigDecimal price2);
...
//Write a native query that returns a movie by name
    ...
    @Query(value = "SELECT * FROM movie WHERE name = ?1", nativeQuery = true)// table name = movie
...
//Write a native query to return all movies where duration exists in the range of duration
    ...
    @Query(value = "SELECT * FROM movie WHERE duration IN ?1", nativeQuery = true)//BETWEEN and IN - correct
