...
public class LicenseController {
    ...
    @GetMapping("/{licenseId}")//2.retrieve license
    public ResponseEntity<License> getLicense(@PathVariable("organizationId") Long organizationId, @PathVariable("licenseId") Long licenseId) throws Exception {
        License license = licenseService.getLicense(licenseId, organizationId);//cant delete bec secure in organiz - controller
        return ResponseEntity.ok(license);
    }

    @PutMapping("/{licenseId}")//create license
